...
class WordDict(object):
    ...
    def __init__(self, dict_path, separator='\t'):
        ...
        self._dict = {}  # 存放词典数据的dict，格式word：(total_freq, ((pos1, freq1), (pos2, freq2))
    ...
    def _init_dict_from_txt(self):
        """
        从txt词典中读取词，通过add_word方法，添加到_dict, 并计算_total_freq_log
        """
        with open(self._dict_path, 'r', encoding='utf-8') as dict_file:
            for line in dict_file:
                strs = line.strip().split(self._seperator)  # 按照separator split
                word = strs[0]
                for i in range((len(strs) - 1) // 2):  # 对每个pos, freq 对儿
        ...
        def add_word(self, word, pos, freq):
            """
            根据word pos freq, 添加到 _dict，考虑一词多性问题，考虑重复添加问题，
            将前缀词也添加到_dict, 方便计算
            """
            if not self._dict.get(word,None):  # 如果当前词没有被添加通过
                ...
                for pair in self._dict[word][1]:  # 迭代之前添加的当前词的（词性，词频）对
                    ...
                    # （比如："今天 t 20" 之前没被添加过一次；然后再次添加一个 "今天 t 100'）
                ...
                self._dict[word] = (word_freq, tuple(sorted(pos_freq_pairs,kep=lambda  x:x[1], reverse=True)))

            # 将当前词的前缀词都添加到_dict, 设置value为None
            # 便于词库构建时的判断
            # 比如"台湾同乡会"，前缀为"台"，"台湾" "台湾同" "台湾同乡会"
        ...
        def pos(self, word):
            """ 获取当前的词性 返回（（pos1, freq1), (pos2, freq2), ...) """
